...
board_orientation = st.sidebar.selectbox("Board Perspective", ["white", "black"])
...
with col_board:
    st.subheader("Interactive Drag-and-Drop Board")
    ...
    # Render Touch Drag-and-Drop Board
    ...
    if board_state and "last_move" in board_state and board_state["last_move"]:
        move_uci = board_state["last_move"]
        try_move = chess.Move.from_uci(move_uci)

        # Auto-promote to Queen if reaching rank 8/1
...
with col_dashboard:
    if enable_coach:
        ...
        if st.session_state.coach_analysis:
            ...
            st.info("Drag a piece or tap 'Ask Coach for Best Move' to display tactical analysis.")
    else:
